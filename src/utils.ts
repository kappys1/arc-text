...
export const splitNode =  (node: HTMLElement, splitter: any) => {
  ...
  return chars.map((char:string) => {
    ...
    parent.insertAdjacentHTML('afterbegin', char === ' ' ? '&nbsp;' : char);getLetterRotations
    return parent;
  });
};
...
export const getLetterRotations = (metrics: any, r: number) => metrics.reduce((data: any, { width } : any) => {
